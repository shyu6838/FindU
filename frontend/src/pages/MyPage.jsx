...
import ProfileEditModal from '../components/ProfileEditModal'; // 💡 모달 컴포넌트 추가
...
const MyPage = ({ onNavigate }) => {
  ...
  const [isProfileEditOpen, setIsProfileEditOpen] = useState(false); // 💡 프로필 수정 모달 상태

  // 💡 프로필 수정 후 화면을 갱신할 수 있도록 데이터 호출 함수를 useCallback으로 분리
  ...
  return (
    <div style={styles.page}>
      ...
      <ProfileEditModal 
        ...
        currentUser={user} 
        onUpdateSuccess={fetchMyPageData} // 수정 성공 시 마이페이지 데이터 갱신
      />
    </div>
  );
};
...
const styles = {
  ...
  editIconBtn: { background: 'none', border: 'none', fontSize: '16px', cursor: 'pointer', padding: '0', color: '#6b7280' }, // 💡 톱니바퀴 스타일
